...
const renderMarkdown = ({ decisionRows, evidence, limitations, paretoRows, runtimeRows, scoreRows }) => {
  ...
  return [
    ...
    '### Runtime summaries',
    '',
    renderMarkdownTable(runtimeRows, ['candidateId', 'latency_median_ms', 'latency_p95_ms', 'latency_p99_ms', 'error_rate', 'throughput_rps', 'availability', 'restart_count', 'cpu_utilization', 'memory_utilization', 'missing']),
    '',
    '### Pareto comparison',
    '',
...
const runtimeRow = (value, sourcePath, reason, manifest) => {
  ...
  return {
    ...
    source: value.source ?? '',
    fixtureProfile: manifest?.fixtureProfile ?? '',
    latency_median_ms: formatValue(summary.latency_median_ms),
