...
A method uses another module's data more than its own owner's data. Move Method, Extract Method, Move Field

## Definition
A method uses another module's data more than its own owner's data.

## Typical signals
Long chains of getters or calculations over a foreign entity.
...
A service should not reach through entity internals to reconstruct a policy owned by the entity's domain.
...
A mapper or read model may intentionally inspect another shape.
