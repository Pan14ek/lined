...
Abstraction exists for hypothetical variation rather than a current force. Collapse Hierarchy, Inline Class, Remove Parameter, Replace Delegation with Composition

## Definition
Abstraction exists for hypothetical variation rather than a current force.

## Typical signals
Single implementation interfaces, unused type parameters, and generic flags.
...
A one-implementation interface is acceptable only when it is a real provider/framework/domain seam, such as CurrentUserProvider or PaidSubscriptionLookupPort.
...
Framework contracts and external ports may be justified with one implementation.
