...
## Purpose
Translates a third-party provider model into Lined stable concepts.

## Applicability
Use for payment, email, calendar, or identity providers with provider-specific errors and identifiers.

## Ownership
Owner: integration/application seam; provider details must not leak into domain code.

## Anti-patterns
Do not introduce an adapter before a provider boundary exists.
