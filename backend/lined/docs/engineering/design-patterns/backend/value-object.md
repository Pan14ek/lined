...
## Purpose
Gives a domain value explicit invariants and semantics without identity.

## Applicability
Use for bounded concepts such as time windows, codes, or validated identifiers.

## Ownership
Owner: the domain that defines the meaning; avoid sharing coincidentally equal primitives.

## Anti-patterns
Do not wrap every primitive or create ceremony for values with no invariant.
