...
## Intent
Make an invariant explicit at the point it must hold.

## Problem
Later code assumes a condition that is otherwise undocumented.

## Preconditions
Add a meaningful assertion/error with context and test the violated case. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
