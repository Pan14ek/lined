...
## Intent
Represent an intentional absence with a safe object contract.

## Problem
Callers repeat null checks for a legitimate no-op case.

## Preconditions
Define neutral behavior, use it at the boundary, and preserve observability. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
