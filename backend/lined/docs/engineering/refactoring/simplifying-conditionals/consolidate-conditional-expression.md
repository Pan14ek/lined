...
## Intent
Combine repeated checks for one outcome behind a named predicate.

## Problem
The same boolean condition appears in multiple branches.

## Preconditions
Create a pure predicate with a domain name and replace exact equivalents. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
