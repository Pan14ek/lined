...
## Intent
Replace a boolean loop/flow flag with structured control flow.

## Problem
A flag exists only to break or select an exit path.

## Preconditions
Use return/break/guard clauses or a named result and remove the flag. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
