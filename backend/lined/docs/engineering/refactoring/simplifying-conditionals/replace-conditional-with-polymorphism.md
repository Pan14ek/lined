...
## Intent
Move stable variant behavior behind a real substitutable seam.

## Problem
Branches repeat variant-specific operations and new variants are expected.

## Preconditions
Define the smallest contract, implement variants, and retain explicit selection. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
