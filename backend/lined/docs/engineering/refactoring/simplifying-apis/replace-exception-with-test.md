...
## Intent
Use a result/testable condition when exceptions are being used for normal branching.

## Problem
Control flow catches an expected exception as a predicate.

## Preconditions
Expose a query or validation result, then reserve exceptions for failure. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
