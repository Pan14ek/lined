...
## Intent
Remove an argument that is unused or derivable from an owned value.

## Problem
Callers construct or pass data that the callee ignores.

## Preconditions
Delete it, migrate callers, and confirm no reflective/API contract needs it. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
