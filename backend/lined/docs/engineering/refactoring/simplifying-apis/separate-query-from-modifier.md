...
## Intent
Split reading from changing state.

## Problem
A method both returns data and mutates persistence/cache.

## Preconditions
Create a pure query and explicit command, then migrate callers. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
