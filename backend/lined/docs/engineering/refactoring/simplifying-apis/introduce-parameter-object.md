...
## Intent
Group parameters with one semantic meaning into a stable value.

## Problem
Repeated signature clusters carry validation or ordering risk.

## Preconditions
Create a named record/value object, validate at the boundary, and migrate callers. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
