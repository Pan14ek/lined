...
## Intent
Pass a value required by a behavior instead of reading hidden state.

## Problem
The method lacks an explicit input needed for correctness.

## Preconditions
Add it at the narrowest owner, migrate callers, and update mocks/tests. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
