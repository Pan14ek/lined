...
## Intent
Derive a value from an owned object instead of requiring callers to pass it.

## Problem
The parameter duplicates state already available at the callee.

## Preconditions
Replace it with the query, then remove stale call-site computation. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
