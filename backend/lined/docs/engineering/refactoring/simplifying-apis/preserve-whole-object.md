...
## Intent
Pass an object whose values are already a cohesive contract.

## Problem
Callers unpack many fields only to repack them for another method.

## Preconditions
Pass the whole object, keep access at the receiving owner, and remove redundant parameters. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
