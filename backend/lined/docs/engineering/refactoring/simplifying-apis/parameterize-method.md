...
## Intent
Unify methods whose only difference is a meaningful parameter.

## Problem
Several methods implement the same algorithm with one varying value.

## Preconditions
Introduce a parameter, validate allowed values, and preserve intention at callers. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
