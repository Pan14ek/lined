...
## Intent
Use distinct method names when options have different semantics.

## Problem
A boolean or enum makes call sites ambiguous and branches diverge.

## Preconditions
Create explicit methods, share only the truly common implementation, and migrate callers. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
