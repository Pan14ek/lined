...
## Intent
Give a method a name that states its domain action and error behavior.

## Problem
Callers need comments or misuse the method because its name is vague.

## Preconditions
Choose the owner vocabulary, update callers and tests, then remove aliases when safe. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
