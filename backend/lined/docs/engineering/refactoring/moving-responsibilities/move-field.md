...
## Intent
Move data to the object whose behavior protects it.

## Problem
A field is read and mutated mainly by another owner.

## Preconditions
Move storage and accessors together, update mappings/persistence, and migrate tests. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
