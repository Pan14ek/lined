...
## Intent
Separate two change reasons that have become coupled.

## Problem
A class has distinct collaborators, data, and tests for separate concepts.

## Preconditions
Define ownership, move a cohesive slice, delegate temporarily if needed, then simplify. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
