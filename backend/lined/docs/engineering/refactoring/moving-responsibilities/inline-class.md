...
## Intent
Remove a class that no longer earns a seam.

## Problem
The deletion test leaves no complexity behind and the class only forwards.

## Preconditions
Move meaningful behavior/data to the owner, update imports, and delete the wrapper. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
