...
## Intent
Move behavior to the module that owns most of its data and invariant.

## Problem
Feature envy or repeated foreign navigation obscures ownership.

## Preconditions
Move with a compatibility call if needed, update callers, then remove the forwarding path. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
