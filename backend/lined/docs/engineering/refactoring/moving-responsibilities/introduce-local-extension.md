...
## Intent
Create a local wrapper around an external type with several missing operations.

## Problem
A foreign type is used repeatedly and changes cannot be made at source.

## Preconditions
Wrap only the needed contract and keep provider details localized. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
