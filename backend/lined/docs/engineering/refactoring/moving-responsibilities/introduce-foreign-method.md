...
## Intent
Add a local extension when a needed behavior belongs to a stable external type.

## Problem
A small missing operation is used by one owner and cannot be added to the foreign type.

## Preconditions
Keep the extension near its caller, name it after the domain concept, and add tests. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
