...
## Intent
Remove a forwarding object that adds no value.

## Problem
A wrapper has no translation, policy, lifecycle, or seam role.

## Preconditions
Inline calls, update callers, and delete the wrapper after diff review. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
