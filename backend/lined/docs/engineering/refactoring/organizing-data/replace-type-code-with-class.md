...
## Intent
Replace a value code with a type that owns behavior or metadata.

## Problem
A code has validation, labels, or rules scattered across conditionals.

## Preconditions
Introduce the type, migrate comparisons, and keep API/DB translation explicit. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
