...
## Intent
Restrict direct field mutation behind an intention-revealing API.

## Problem
Callers can put an object into invalid state.

## Preconditions
Change visibility, add controlled access, migrate callers, and preserve serialization contracts. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
