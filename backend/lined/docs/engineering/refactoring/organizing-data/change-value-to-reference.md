...
## Intent
Replace duplicated value copies with a shared identity.

## Problem
Updates must be observed by all consumers or the value has stable identity.

## Preconditions
Introduce the reference, migrate lifecycle and persistence, and protect deletion behavior. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
