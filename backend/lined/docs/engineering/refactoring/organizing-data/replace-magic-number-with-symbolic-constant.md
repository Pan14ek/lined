...
## Intent
Name a value that encodes a domain or operational concept.

## Problem
A timeout, limit, protocol code, or threshold is repeated or unexplained.

## Preconditions
Use an existing framework/type first, then a narrowly owned constant, and update tests. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
