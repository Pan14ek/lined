...
## Intent
Collapse a subclass hierarchy when variants differ only by data.

## Problem
Subclasses contain no distinct invariant or behavior.

## Preconditions
Move distinguishing values to fields, remove hierarchy, and preserve serialization. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
