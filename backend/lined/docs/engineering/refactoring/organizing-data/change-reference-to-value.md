...
## Intent
Copy a reference into an immutable value when sharing creates coupling.

## Problem
The referenced object is immutable or independently owned and only its value matters.

## Preconditions
Define value equality, copy at the boundary, and remove unnecessary navigation. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
