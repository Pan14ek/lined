...
## Intent
Keep a local read model synchronized with an authoritative source.

## Problem
UI or reporting needs a projection with different lifecycle/shape.

## Preconditions
Define source, update timing, failure recovery, and consistency tests before copying. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
