...
## Intent
Route field access through methods to protect future invariants.

## Problem
Direct reads/writes bypass validation or make a move difficult.

## Preconditions
Introduce accessors, migrate internal uses, then add invariant checks if needed. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
