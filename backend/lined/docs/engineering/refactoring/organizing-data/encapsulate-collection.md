...
## Intent
Prevent callers from mutating an owner's collection directly.

## Problem
A collection invariant is bypassed by add/remove/replace operations.

## Preconditions
Return an unmodifiable view or copy and expose named operations. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
