...
## Intent
Move behavior used by only one subtype to that subtype.

## Problem
A parent exposes a method irrelevant to most variants.

## Preconditions
Move it, narrow the parent contract, and update callers. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
