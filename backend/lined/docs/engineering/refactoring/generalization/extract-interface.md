...
## Intent
Name a real seam for multiple implementations or substitution.

## Problem
Callers need a stable contract independent of a concrete implementation.

## Preconditions
Extract the smallest contract, update dependency direction, and keep one implementation only when a real seam exists. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
