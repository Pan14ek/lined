...
## Intent
Move identical state to a common owner.

## Problem
Sibling implementations duplicate one field and its invariant.

## Preconditions
Confirm same meaning, move storage, update constructors/mapping, and test all variants. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
