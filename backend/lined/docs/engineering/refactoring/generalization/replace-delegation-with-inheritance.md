...
## Intent
Use inheritance when true substitutability and stable shared implementation exist.

## Problem
A wrapper adds no policy and all callers require the full base contract.

## Preconditions
Move shared behavior carefully and preserve extension points. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
