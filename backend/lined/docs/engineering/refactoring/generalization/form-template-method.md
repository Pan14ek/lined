...
## Intent
Fix algorithm ordering while allowing bounded variant steps.

## Problem
Variants share lifecycle but differ in a few operations.

## Preconditions
Define the invariant skeleton, delegate explicit steps, and test ordering. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
