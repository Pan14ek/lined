...
## Intent
Replace fragile subclassing with an owned collaborator.

## Problem
Subclassing leaks parent API or variation is runtime-configurable.

## Preconditions
Wrap the collaborator, forward only needed behavior, then remove inheritance. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
