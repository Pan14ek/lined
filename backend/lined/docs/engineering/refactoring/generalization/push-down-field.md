...
## Intent
Move state used by one subtype to that subtype.

## Problem
A parent field is meaningless for other variants.

## Preconditions
Move mapping/storage carefully and preserve external contracts. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
