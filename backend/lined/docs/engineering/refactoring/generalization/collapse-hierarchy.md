...
## Intent
Remove a hierarchy that no longer represents meaningful variation.

## Problem
Subclasses are empty, identical, or only preserve history.

## Preconditions
Inline behavior/data, migrate callers, and delete the unused base. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
