...
## Intent
Create a subtype for a stable variant with extra behavior/state.

## Problem
A class has conditional behavior that belongs to one durable variant.

## Preconditions
Move variant behavior and preserve the base contract. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
