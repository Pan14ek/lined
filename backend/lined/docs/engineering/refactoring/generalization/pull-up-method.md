...
## Intent
Move identical behavior to a common owner.

## Problem
Siblings duplicate a complete method with the same contract.

## Preconditions
Verify dependencies and error semantics, move it, and simplify subclasses. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
