...
## Intent
Move a cohesive fragment behind a named seam so the caller states intent.

## Problem
A method mixes orchestration with one understandable calculation or side effect.

## Preconditions
Select a contiguous or conceptually cohesive fragment, name inputs/outputs, move it, then compare behavior. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
