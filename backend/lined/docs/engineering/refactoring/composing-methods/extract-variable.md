...
## Intent
Name an expression whose meaning or repeated use is important.

## Problem
A boolean, query predicate, or transformation is hard to read inline.

## Preconditions
Choose a semantic name, assign once, replace occurrences, and keep scope local. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
