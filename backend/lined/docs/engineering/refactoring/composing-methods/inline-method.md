...
## Intent
Remove a method whose body is no clearer than its calls.

## Problem
The method is a pass-through or its name does not add a meaningful contract.

## Preconditions
Replace calls with the body, compile, run tests, and delete only after callers are inspected. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
