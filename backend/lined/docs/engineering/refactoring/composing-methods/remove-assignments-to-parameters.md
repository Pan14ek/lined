...
## Intent
Avoid mutating input parameters to make data flow explicit.

## Problem
A parameter is reassigned and callers cannot distinguish input from working value.

## Preconditions
Introduce a local, preserve the parameter, and verify null/immutability behavior. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
