...
## Intent
Remove a temporary that only aliases one expression.

## Problem
A temporary obscures a simple calculation or prevents a later move.

## Preconditions
Confirm the expression has no side effects, replace uses, and run tests. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
