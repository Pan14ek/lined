...
## Intent
Package a complex method's data and steps into a focused object.

## Problem
Many locals and parameters make extraction awkward and the algorithm has its own lifecycle.

## Preconditions
Move inputs into a small object, move steps, expose one operation, and keep ownership local. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
