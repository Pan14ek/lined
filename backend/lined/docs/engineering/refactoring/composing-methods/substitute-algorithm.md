...
## Intent
Replace an implementation with a clearer equivalent algorithm.

## Problem
The current algorithm is hard to verify or a known simpler approach exists.

## Preconditions
Write behavior tests, implement the replacement, compare edge cases, then remove old code. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
