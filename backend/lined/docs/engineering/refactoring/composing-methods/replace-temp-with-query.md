...
## Intent
Turn a repeated calculation into a named query.

## Problem
A temporary represents a stable concept used in multiple branches.

## Preconditions
Create a side-effect-free query, replace the temporary, and verify no time/state drift. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
