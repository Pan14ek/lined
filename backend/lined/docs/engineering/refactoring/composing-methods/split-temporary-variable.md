...
## Intent
Give one temporary variable one responsibility.

## Problem
The same variable is assigned for unrelated purposes.

## Preconditions
Introduce distinct names at each assignment and update uses in small steps. Confirm ownership, search the Pattern Registry, and add characterization tests if the change can affect behavior.
