...
- Create the local-only `lined-postgres` Secret as described in the kind
  baseline guide.
- Keep the existing k6 workload from `docs/load-test-baseline.md` available for
...
`docs/load-test-baseline.md` for actual comparison runs.
...
- Actuator Prometheus request and JVM metrics from
  `docs/runtime-metrics-baseline.md`.
