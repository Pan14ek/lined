...
Machine-readable consumers should use role arrays instead of parsing prose.

| Role | Meaning |
|------|---------|
| `hard-constraint` | A validity guardrail such as latency, error rate, readiness, or restart count. |
| `warning` | A pressure signal that requires review but does not invalidate a run by itself. |
| `objective` | A signal that may be optimized or compared across valid scenario runs. |
| `context-signal` | Evidence that explains behavior but is not inherently good or bad. |
| `validation-evidence` | Evidence used to prove the run measured a ready and comparable system. |
| `exploratory` | Evidence collected for pressure exploration rather than stable comparison. |
...
| `slo_or_constraint_role` | Human-readable summary of the scenario role. |
| `slo_or_constraint_roles` | Machine-readable role values from this document. |
| `evidence_source` | Primary artifact or telemetry source used to support the scenario. |
| `evidence_kind` | Primary evidence taxonomy value for the scenario's measurable evidence. |
| `provenance_source` | Manifest or run metadata source used for audit and exclusion reasoning. |
...
| `local-stress-exploration` | Reliability / capacity behavior | Local stress fixture with ramping virtual users. | latency, error rate, throughput, CPU/memory pressure, HPA state when applicable. | Exploratory evidence only; v1 hard stable-comparison thresholds do not apply. | k6 summary, Kubernetes state, and optional runtime summary when intentionally accepted; runtime summary manifest for provenance only. |

## Relationship To Existing Artifacts

- `docs/research/platform/runtime-scenario-summaries.md` defines how local scenario runs produce
  `runtime-summary.json` and `runtime-summary-manifest.json`.
- `docs/research/platform/hpa-resource-scenarios.md` defines the local kind deployment variants:
  `fixed-small`, `fixed-medium`, `replicas-2`, and `hpa-cpu`.
- `docs/research/platform/slo-constraint-thresholds.md` and
  `load-tests/runtime-scenarios/slo-thresholds-v1.json` define v1 validity and
  warning classifications.
- `docs/research/fitness/runtime-fitness-extension.md` defines runtime-summary fields and
